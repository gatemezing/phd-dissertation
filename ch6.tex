\chapter{Best Practices for Publishing Linked Data}
\label{ch:ch6}
...
\item Datalift vision \cite{scharffe_2012}, the process is divided into 3 principal phases: modeling the data , publishing and exploitation. Figure \ref{fig:datalift-ld} depicts the different steps, closed to the architecture of the platform:
	\begin{enumerate}
...
Although the process of lifting raw data to interlinked data in Datalift seems to be linear, it can also be cyclic with a maintenance task in the life cycle. Table \ref{tab:lifecycle} summarizes the ten best practices to be taken into account to publish data as Linked Data on the Web.
...
In this chapter, we mainly describe our contributions on concrete implementations regarding how to use standard vocabularies (Section \ref{sec:lov-approach}, Section \ref{sec:prefharmoni} and Section \ref{sec:vocabranking}), how to model and convert the data (Section \ref{sec:data2rdf}, Step \#3 \& Step \#7) and practical consideration on how to specify appropriate license on vocabularies and datasets (Section \ref{sec:checker}, cf. Step \#4).  
...
\subsection{Statistics about licensed vocabularies}
...
\subsection{Related work about licenses in the Web of Data}
