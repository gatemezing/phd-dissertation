...
\begin{enumerate}
\item \textbf{Search form:} The interface retrieves the location automatically ( \'{a} la Google Maps API fashion) , and offers choices based on the School type: First degree / second degree. When choosing first degree, the user can further select one of (primary school, elementary school or other). For the second degree, apart from looking for one of College, high school or other, the user can look for public or private schools. The search button launches the query behind the scene for retrieving the collection of data matching the user's criteria.
...
\subsubsection{Modeling Events and Applications in RDF}
...
\subsubsection{Improving the model for specifying winners}
...
\subsubsection{Universal JavaScript plugin for RDF population}
\label{sec:universal}
...
\subsubsection{Creating the Knowledge-base for Past Events}
