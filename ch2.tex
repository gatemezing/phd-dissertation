...
\begin{description}
\item [Requirement \#1 -Model:] Model the existing data according to an existing vocabulary covering the dataset.
\item[Requirement \#2 -Provenance:] Add relevance metadata on the published dataset (spatial coverage, licensing, authorship, etc.)
\item[Requirement \#3 Stable URIs]: Define a policy to have stable URIs for identifying uniquely each address.
\item[Requirement \#4 -Access to data:] Provide primarily a frequent dump of the dataset in RDF.
