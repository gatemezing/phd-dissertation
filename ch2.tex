...
\lstset{basicstyle=\scriptsize, backgroundcolor=\color{white}, frame=shadowbox, caption = {Sample of structured geometry of the city of Nice.}, language=ttl, label=lst:nicegeomrdf, captionpos=b}
\begin{lstlisting}
...
In \cite{anamaria08}, Olteanu mentions many different forms of interlinking geospatial datasets. We highlight below four important criteria to be taken into account in the process of interlinking two geospatial datasets:
\begin{itemize}
...
\caption{The most used distances on geometrical primitives: (a) the Euclidean distance and (b) the Hausdorff distance.}
}
\end{figure}
...
\subsection{GeOnto Alignment Process Scenario}
 ...
 %\subsection{Schema Alignment of Existing Vocabularies}                                        
...
%\subsubsection{GeOnto Alignment Process}
...
\label{sec:geoknow}
%posible title:GeoKnow Stack Framework%
...
%\subsection{Datalift Platform}\label{sec:datalift}
...
\begin{description}
\item [\ding{117}~Requirement 1-Model:] Model the existing data according to an existing vocabulary, generic enough to cover the scope of the existing dataset.
\item[\ding{117}~Requirement 2-Provenance:] Add relevance metadata on the published dataset  such as provenance, spatial coverage, licensing, authorship, etc.
\item[\ding{117}~Requirement 3-Stable URIs:] Define a policy to provide and ensure stable URIs for identifying uniquely each address entity on the Web.
\item[\ding{117}~Requirement 4-Interconnection:] Find relevant dataset already published on the Web to which interconnect for better discoverability.

\item[\ding{117}~Requirement 5-Access to data:] Provide primarily a frequent dump of the dataset in RDF.

\item[\ding{117}~Requirement 6-GeoSPARQL interoperability:] Provide different representations of geometries of locations that are interoperable with GeoSPARQL standard.
...
\lstset{basicstyle=\scriptsize, backgroundcolor=\color{white}, frame=shadowbox, caption= {The metadata information used in the BANO2RDF dataset.}, language=json, label=metadatabano2rdf, captionpos=b}
\begin{lstlisting}
...
\lstset{basicstyle=\scriptsize, backgroundcolor=\color{white}, frame=shadowbox, caption= {This LIMES script is used to interconnect the Bano2RDF dataset located in the region of Marseille with buildings in LinkedGeoData.}, language=XML, label=scriptLIMES, captionpos=b}
\begin{lstlisting}
<LIMES>
...
\begin{tabular}{lrl}
\specialrule{1pt}{1pt}{1pt}
 \textbf{Dataset}	& 	  \textbf{\#Triples} & \textbf{Domain}\\ \specialrule{1pt}{1pt}{1pt}
...
\begin{tabular}{lll}
\specialrule{1pt}{1pt}{1pt}
 \textbf{URI}	& 	\textbf{Label}  & \textbf{Distance} \\ \specialrule{1pt}{1pt}{1pt}
\url{lgdr:node1645286605} &  Fondation Sophia Antipolis & 1.43949  \\
\url{lgdr:node1274078114} &	 Caisse d'allocations familiales & 4.12131 \\
\url{lgdr:node1082377863	} & Caisse des \'{e}coles & 5.06445 \\
\url{lgdr:node1082377868	} & 	Centre Administratif & 5.14575 \\
\url{lgdr:node960122458}	 &  Tr\'{e}sorerie de Mougins & 5.54149 \\
\url{lgdr:node471974902}	 & 	Centre d'information jeunesse & 7.07093 \\
\url{lgdr:node2209129663} & Not available & 7.59737

		\\ \specialrule{1pt}{1pt}{1pt}
\end{tabular}
\caption{Results of the public buildings 10 km around EURECOM from LinkedGeoData endpoint. \texttt{lgdr} represents the base URI for \url{http://linkedgeodata.org/triplify/} }
\label{tab:lgd-query}
}
\end{table}
